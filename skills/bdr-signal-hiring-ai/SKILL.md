---
name: bdr-signal-hiring-ai
description: >
  Detect ICP-fit companies that are actively hiring for Artificial Intelligence roles — from
  AI leadership and delivery roles (Chief AI Officer, Head of AI, Head of Data Science,
  Machine Learning Engineer, GenAI Lead) to the AI-governance operating-model roles that are
  the strongest form of this signal (AI Governance Lead, Model Risk / Model Validation, Responsible
  AI, AI Policy). The full keyword list is client-managed in Notion, never hardcoded here.
  Fetches company ICP criteria and the AI hiring keyword list fresh from the team's Notion
  reference page, queries Apollo for companies matching the ICP profile, checks their active job
  postings for matching openings (capturing the posting date so signal freshness is known),
  surfaces the ICP decision-maker(s) at those companies, then enriches the qualified leads for
  real contact info. Output is a JSON payload ready for the zoho-crud-lead skill.
  This is the Artificial Intelligence sibling of bdr-signal-hiring-data and bdr-signal-hiring-risk —
  same mechanism, its own client-managed reference page.
  Use when asked to find companies hiring for AI, GenAI, machine learning, data science, AI
  governance, or model risk roles, run the AI hiring signal scan, detect AI hiring activity at
  ICP companies, or find who is building out an AI or model-risk team.
  Triggers on: "run the AI hiring scan", "which ICP companies are hiring for AI", "AI hiring
  signal", "who's hiring for AI governance / model risk", "GenAI hiring signal",
  "bdr-signal-hiring-ai".
...
# Eclipse BDR — Signal: Company Hiring (Artificial Intelligence)

Surface ICP-fit companies with active AI job postings — AI leadership, GenAI/ML delivery, and
AI-governance operating-model roles alike — then find the ICP decision-maker at those companies
and enrich them for real contact info. The structured JSON payload hands off to `zoho-crud-lead`
to write to Inbound (Leads); the user-facing deliverable is the templated report (see 📐 Output
Standard below).
...
AI delivery and AI governance hiring are **one consolidated signal**. The precision distinction
lives in the keyword list, which the client manages in Notion (Section B of the reference page) —
not in separate skills and not hardcoded here.

This is the **Artificial Intelligence** sibling of `bdr-signal-hiring-data` (Data) and
`bdr-signal-hiring-risk` (Risk & Regulatory). Same mechanism, same output shape, same
...
**Reference (Notion) — Skill References - Signal: Company Hiring (AI):**
`https://app.notion.com/p/3914e4751c3a81228cb3c4da2a7711d6`
(in the **Skill References** database)

If that URL fails (page moved or renamed), locate it with `notion-search` (query:
`"Signal: Company Hiring (AI)"`) and read the page it returns instead.
...
- **Section B — AI Role Keywords:** `ai_role_keywords` — the full list of job title keywords to
  match against open postings (covers both AI delivery roles and AI-governance operating-model
...
- `ai_role_keywords` — keywords to match against job posting titles (case-insensitive substring
  match)
...
## Step 2: Find ICP Companies with AI Job Postings
...
`ai_role_keywords` (case-insensitive substring match — match on **any one** keyword, never require
all of them).

**Capture the signal mechanism and freshness for every matched posting** (the team wants to know
how old each signal is — fresher postings are higher-intent):
...
actually pitch** — the AI decision-maker / leader at the signal company (e.g. the Chief AI Officer,
CDAO, Head of AI, AI Governance Lead, Head of Model Risk). This is **not** the recruiter or whoever
posted the req; it is the ICP buyer whose team the new headcount rolls up into. A company staffing
an AI program needs an owner for it, and that owner is the lead.
...
| Person is C-suite (Chief AI Officer, CDAO, CIO, CTO, Chief Innovation Officer) | +3 |
| Person is VP-level | +2 |
| Person is Director-level | +1 |
| Matched postings include an AI-governance / model-risk role (program being governed, not just piloted) | +1 |
...
**ICP role match:**
- C-suite AI/analytics title → `"Primary ICP"`
- VP or Director AI title, or AI Governance / Model Risk lead → `"Secondary ICP"`
...
contact data. **Enrich qualified leads only** — never the full scanned set — to keep credit spend
tight.
...
(phone reveal may be metered separately on some plans). This is the only credit-consuming step in
the skill; the scan steps above are not bulk-credit enrichments. Always report the number of records
enriched in the run summary so credit usage is visible.
...
If the team explicitly wants a no-cost dry run, this step may be skipped; in that case last names
stay masked and email/phone stay `null`. State clearly in the summary that enrichment was skipped.
...
| `practice_areas` | `Practice_Areas_In_Scope` | Always `["Artificial Intelligence"]` (map to the Zoho picklist value) |
...
> "[Company] is actively hiring for AI roles ([list matched posting titles, max 3]; latest posted
> [signal_age_days] days ago). [First name] is [title] there — surfaced as an Artificial Intelligence
> signal on [scan date]."

If the matched postings are predominantly AI-governance / model-risk roles (e.g. AI Governance,
Model Risk, Responsible AI, Model Validation), note it — this is the strongest form of the signal:

> "[Company] is staffing an AI governance / model-risk program ([list matched posting titles, max
> 3]; latest posted [signal_age_days] days ago). [First name] is [title] there — surfaced as an
> Artificial Intelligence signal on [scan date]."
...
```json
{
  ...
  "crm_payload": {
    "generated_by": "bdr-signal-hiring-ai",
    ...
    "leads": [
      {
        ...
        "fields": {
          ...
          "Practice_Areas_In_Scope": ["Artificial Intelligence"],
...
This skill's detection mechanism: active Artificial Intelligence role job postings (AI
leadership, GenAI / machine-learning delivery, and AI-governance / model-risk operating-model
roles) at ICP-fit companies, via the Apollo job-postings endpoint.
...
1. Query the **Signal Library** data source
   ...
   via `data_source_url`, using this skill's own name (`bdr-signal-hiring-ai`) and these
   coverage terms: "AI hiring", "AI governance", "model risk", "machine learning",
   "responsible AI". (SQL via `notion-query-data-sources` is plan-gated on this workspace — do
   not rely on it.)
...
- **Name:** `bdr-signal-hiring-ai — <YYYY-MM-DD> — <short descriptor of the outcome>`
- **Select:** `bdr-signal-hiring-ai`
...
**Signal:** A company actively hiring for AI roles is standing up or scaling an AI capability —
high-intent signal that they may need external advisory support for AI readiness, governance,
model risk, or an AI operating model. AI-governance and model-risk postings (AI Governance Lead,
Model Risk, Model Validation, Responsible AI) are the strongest form of this signal: per the Signal
Library (SIG-023), they show AI is being **governed**, not just piloted. Hidden hypothesis: the
company is scaling AI use cases but lacks the data readiness, governance, and control operating model
to do it responsibly. **Signal freshness matters** — a posting from this week is a hotter trigger
than one from three months ago, so the posting date is captured and carried through to the lead.

**Who we surface:** the AI **decision-maker** at the signal company (Chief AI Officer / CDAO / Head
of AI / AI Governance Lead / Head of Model Risk) — the buyer for the engagement — not the person who
posted the job.

**Eclipse wedge:** AI readiness assessment, AI use-case prioritization, governed AI roadmap, AI
governance frameworks, model-risk and control design, data readiness for AI, and AI operating-model
support — turning new AI headcount into a governed, value-producing program.

**Family:** The Artificial Intelligence sibling in the per-practice hiring set —
`bdr-signal-hiring-data` (Data), `bdr-signal-hiring-risk` (Risk & Regulatory), and this skill (AI).
Same mechanism and output shape; each reads its own client-managed Notion reference page.
