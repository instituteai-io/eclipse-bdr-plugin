...
- Teams renders `*single asterisks*` as **bold** and `_underscores_` as *italic*; `**double asterisks**`, Markdown tables, and headings do NOT render.
- Power Automate collapses single newlines — use a **blank line (`\n\n`)** between every block **and every bullet** to get visible spacing, or the message reads as a wall of text.
