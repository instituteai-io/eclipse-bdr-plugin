...
description: >
  ...
  data/analytics/risk-reporting/AI-governance (reactive only — scans a pasted RFP URL; proactive
  procurement-portal detection is a known gap per Signal Library SIG-027), and investor-deck /
  annual-report data-or-AI narrative.
...
ICP firmographics are **not hardcoded** — fetch them fresh on every run from the **01 ICP & Buyer
Persona Analysis** Notion page: `https://app.notion.com/p/3904e4751c3a81dca87af64d0ea85822`
...
**Source preference & fetch caveats** (learned from live runs): prefer the **company-owned
investor-relations or newsroom page** over aggregators — it fetches reliably and is the primary
source. SEC EDGAR (`sec.gov`) returns HTTP 403 unless fetched with a Bash `curl` that declares a
User-Agent **including a contact email** (`curl -sL -A "EclipseBDR/1.0 (contact: <team email>)" -m 45`);
some newswire hosts (e.g. BusinessWire) time out WebFetch — retry once with the same curl form, then
fall back to the company's own copy of the release.
...
fresh from the 01 ICP & Buyer Persona Analysis page rather than a dedicated keyword reference page.
