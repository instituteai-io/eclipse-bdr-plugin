...
| `signal_type` | `Signal_Type` | Always `"New Executive Sponsor"` ✅ Live in Zoho |
| `signal_mechanism` | `Signal_Mechanism` | Always `"Apollo role-start inference"` No Zoho field — `zoho-crud-lead` folds it into `Signal_Summary` |
| `signal_date` | `Signal_Date` | Date WE detected the signal = scan date ✅ Live in Zoho |
| `signal_source_date` | `Signal_Source_Date` | The exec's current-role start date; how old the appointment is No Zoho field — `zoho-crud-lead` folds it into `Signal_Summary` |
| `signal_summary` | `Signal_Summary` | Generated in Step 7 ✅ Live in Zoho |
| `needs_confirmation` | `Signal_Needs_Confirmation` | `true` when `require_web_confirmation` — human confirms via press/leadership page before outreach No Zoho field — `zoho-crud-lead` folds it into `Signal_Summary` |
    ...
    "leads": [
      {
        ...
        "fields": {
          ...
          "Signal_Type": "New Executive Sponsor",
          "Signal_Mechanism": "Apollo role-start inference",
          "Signal_Date": "<today YYYY-MM-DD>",
          "Signal_Source_Date": "<role start date or null>",
