...
- `require_apollo_id` — `true` / `false`: whether to skip leads missing `Apollo_Contact_ID`
...
`id`, `First_Name`, `Last_Name`, `Company`, `Designation`, `Apollo_Contact_ID`, `Signal_Date`

If `require_apollo_id` is `true` (default): skip any lead where `Apollo_Contact_ID` is blank.
...
|---|---|
| `Apollo_Contact_ID` | `id` (direct lookup if present) |
