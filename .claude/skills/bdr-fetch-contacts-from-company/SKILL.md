...
description: >
  ...
  or apollo). Reads the practice's ICP buyer titles and seniority fresh from the practice's
  department reference page in Notion (never hardcoded), then queries the provider for matching contacts AT that company and returns
...
The practice → **buyer titles + seniority** mapping is **not hardcoded**. Fetch it fresh each run
(via `notion-fetch`) from the practice's **department reference page** in the Skill References
database — the same pages the hiring skills use, so titles are managed in exactly one place:

| Practice | Department page |
|---|---|
| Data Management | `https://app.notion.com/p/23e4e4751c3a827480e10109d6538c63` — Signal: Company Hiring (Data) |
| Risk & Regulatory | `https://app.notion.com/p/6ce4e4751c3a8331828b818f9efe37b4` — Signal: Company Hiring (Risk) |
| Transformation | `https://app.notion.com/p/3914e4751c3a8147adbac414b716d10f` — Signal: Company Hiring (Transformation) |
| Artificial Intelligence | `https://app.notion.com/p/3914e4751c3a81228cb3c4da2a7711d6` — Signal: Company Hiring (AI) |

Use **Section A — ICP Company & Contact Criteria** on the matching page: its **Job Titles** list and
**Seniority** line. (Section C on the same page carries the fuller practice ICP detail — personas,
fit indicators, extended title taxonomy — if broader context is needed.) If the page is unreachable,
**stop and tell the user** — do not guess titles.
...
Fetch the practice's department reference page; extract Section A's **Job Titles** and **Seniority**
...
```json
{
  ...
  "practice": "<practice>",
  "enriched": true,
  "titles_source": "<the department page URL used for this practice>",
  "company": {
