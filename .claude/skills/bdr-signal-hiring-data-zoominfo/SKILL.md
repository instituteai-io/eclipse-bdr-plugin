...
decision-maker at those companies and enrich them for real contact info. The structured JSON
payload hands off to `zoho-crud-lead` to write to Inbound (Leads); the user-facing deliverable
is the templated report (see 📐 Output Standard below).
...
| `zoominfo_contact_id` | `ZoomInfo_Contact_ID` | ✖ Not in Zoho (schema is final; ZoomInfo `personId`) — `zoho-crud-lead` drops it; dedup falls back to Email / name+company. NOTE: can be a **negative** integer — Zoho field must be text/long and tolerate the sign |
| `signal_type` | `Signal_Type` | Always `"Company Hiring"` ✅ Live in Zoho |
| `signal_mechanism` | `Signal_Mechanism` | `"ZoomInfo Scoop — Open Position"` / `"… Hiring Plans"` No Zoho field — `zoho-crud-lead` folds it into `Signal_Summary` |
| `signal_date` | `Signal_Date` | Date WE detected the signal = scan date ✅ Live in Zoho |
| `signal_source_date` | `Signal_Source_Date` | The scoop's own publish date (`latest_signal_date`); how old the signal actually is No Zoho field — `zoho-crud-lead` folds it into `Signal_Summary` |
...
This skill's detection mechanism: active Data and data-governance hiring at ICP-fit companies,
via ZoomInfo Scoops (Open Position / Hiring Plans scoop types).
...
1. Query the **Signal Library** data source
   ...
   via `data_source_url`, using this skill's own name (`bdr-signal-hiring-data-zoominfo`) and
   these coverage terms: "data hiring", "data governance hiring", "data steward", "metadata",
...
search and enrichment), build the template's **Sources Used** table:
...
- **Name:** `bdr-signal-hiring-data-zoominfo — <YYYY-MM-DD> — <short descriptor of the outcome>`
- **Select:** `bdr-signal-hiring-data-zoominfo`
